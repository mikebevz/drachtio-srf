...
const SipError = require('./sip_error').SipError;
const async = require('async') ;
const deprecate = require('deprecate');
const debug = require('debug')('drachtio:srf') ;
const Socket = require('net').Socket;
const { DialogDirection, DialogState } = require('./types');
const noop = () => {};
...
class Srf extends Emitter {
  ...
  constructor(app) {
    ...
    if (!this._app) {
      ...
      if (typeof app === 'object' && !Array.isArray(app)) {
        assert.strictEqual(typeof app.host,  'string', 'invalid drachtio connection opts') ;
  ...
  on(event, fn) {
    ...
    if (_.isString(event) && 0 === event.indexOf('cdr:')) {
  ...
  * @returns {Srf|Promise} returns a Promise if no callback is supplied, otherwise the Srf object
  ...
  endSession(...args) {
    return this._app.endSession(args);
  }

  disconnect(...args) {
    return this._app.disconnect(args);
  }

  set(...args) {
    return this._app.set(args);
  }

  get(...args) {
    return this._app.get(args);
  }
  ...
  get locals() {
    return this._app.locals;
  }

  set locals(locals) {
    this._app.locals = locals;
  }

  get idle() {
    return this._app.idle;
  }
...
*   @name Srf#endSession
*   @method
*   @param  {Request|Response} msg - SIP request or response object
* @example
* const Srf = require('drachtio-srf');
* const srf = new Srf();
*
